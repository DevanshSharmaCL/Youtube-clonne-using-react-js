...
const mainlinks = [
    ...
    },
    {
        ...
        name: "Subscriptions" // Corrected typo from "Subscription"
    }
];
...
const otherLink = [
    ...
    },
    {
        ...
        name: "Playlists" // Corrected to plural like YouTube
    },
    {
        icon: <GoVideo className="w-6 h-6" />,
        name: "Your videos" // Capitalized like YouTube
    },
    {
        icon: <MdOutlineWatchLater className="w-6 h-6" />,
        name: "Watch later" // Capitalized like YouTube
    },
    {
        icon: <BiLike className="w-6 h-6" />,
        name: "Liked videos" // Capitalized like YouTube
    }
];
...
const SideBar = () => {
  return (
    <div className="w-[240px] h-screen fixed top-0 left-0 bg-white overflow-y-auto text-[#0f0f0f] font-roboto">
      {/* Main Links Section */}
      <div className="pt-[56px] pb-2"> {/* Adjusted padding for top bar height */}
        {mainlinks.map(({ icon, name }) => (
          <a
            href="#"
            key={name}
            className="flex items-center w-full pl-6 pr-3 py-3 hover:bg-[#f2f2f2] active:bg-[#e5e5e5] transition-colors duration-100"
          >
